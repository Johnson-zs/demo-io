...
void default_event_handler::handle(fs_event event) {
    ...
    if (event.src.find("/.") == std::string::npos) {
        // spdlog::info("src 不是隐藏文件");
        ;
    }
    else if (!event.dst.empty() && event.dst.find("/.") == std::string::npos) {
        // spdlog::info("dst 不是隐藏文件");
        ;
    }
    else {
        // spdlog::info("发现是隐藏文件");
    ...
    // spdlog::info("home: {}", home);
    if (event.src.rfind(home) != 0 && event.dst.rfind(home) != 0){
        // spdlog::info("路径不是以主目录路径开头");
        return;
    }
    bool ignored = false;
